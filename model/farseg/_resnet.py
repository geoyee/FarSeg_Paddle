...
from .param_init import kaiming_normal_init, constant_init
        ...
        if zero_init_residual:
            for m in self.sublayers():
                if isinstance(m, Bottleneck):
                    constant_init(m.bn3.weight, value=0)
                elif isinstance(m, BasicBlock):
                    constant_init(m.bn2.weight, value=0)
...
def _resnet(arch, block, layers, pretrained, **kwargs):
    ...
    if pretrained:
        state_dict = get_weights_path_from_url(model_urls[arch][0],
                                               model_urls[arch][1])
        param = paddle.load(state_dict)
