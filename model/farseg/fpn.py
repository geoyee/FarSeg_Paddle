...
from .param_init import kaiming_uniform_init, constant_init
...
def conv_with_kaiming_uniform(use_gn=False, use_relu=False):
    def make_conv(in_channels, out_channels, kernel_size, stride=1, dilation=1):
        ...
        # corresponds to kaiming_uniform_ in paddle
        kaiming_uniform_init(conv.weight)
        if not use_gn:
            constant_init(conv.bias, value=0)
        module = [conv, ]
        if use_gn:
...
class LastLevelP6P7(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        for module in [self.p6, self.p7]:
            kaiming_uniform_init(module.weight)
            constant_init(module.bias, value=0)
